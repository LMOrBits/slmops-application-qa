...
def level_filter(level="DEBUG"):
    def is_level(record):
        return record["level"].no >= logger.level(level).no or record["level"].name == "DEV"
    return is_level
...
def get_logger(**kwargs) -> Logger:
    ...
    if kwargs:
        ...
        new_logger.add(path,format= log_format + " | {extra}" , rotation="100 MB",retention="10 days",level="ERROR" , backtrace=True,diagnose=True )
        new_logger.add(sys.stdout, format= log_format + " | {extra} ", filter=level_filter() )
        return new_logger
    else: 
        logger.add(path,format=log_format , rotation="100 MB",retention="10 days",level="ERROR" , backtrace=True )
    ...
    def decorator(func):
        if asyncio.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args, **kwargs):
                ...
                end_time = time.time()
                logger.log("TIME",
                    f"Called {func.__name__}, took {end_time - start_time:.2f}s"
                )
                return result
